...
import { useDocumentRevisionStore } from '../stores'
...
const emit = defineEmits<{
  'compare-current': [revisionId: string, revisionNumber: number]
  restore: [revisionId: string, content: string]
}>()

const store = useDocumentRevisionStore()
...
const SOURCE_MAP: Record<string, { label: string; cls: string }> = {
  ...
  ai_enhance: { label: '改写', cls: 'source-enhance' },
  ai_continue: { label: '继续', cls: 'source-continue' },
  restore: { label: '恢复', cls: 'source-other' },
}

const sortedRevisions = computed(() =>
  [...store.revisions].sort((a, b) => b.versionNumber - a.versionNumber),
)
...
async function handleRestore(v: DocumentRevision) {
  const confirmed = window.confirm(`确定恢复到修订 v${v.versionNumber} 吗？当前内容将被替换。`)
  if (!confirmed) return
  const content = v.content ?? await store.getRevisionContent(props.projectId, props.documentId, v.id)
  if (content !== null) {
...
watch(
  () => props.documentId,
  () => {
    store.loadRevisions(props.projectId, props.documentId)
  },
  { immediate: true },
)
</script>
...
<template>
  <div class="version-panel">
    <div class="version-panel-header">
      <span class="panel-title">修订历史</span>
    </div>
    ...
    <div v-else-if="sortedRevisions.length === 0" class="version-empty">
      暂无修订记录
    </div>

    <ul v-else class="version-list">
      <li
        v-for="v in sortedRevisions"
        :key="v.id"
        class="version-item"
      >
