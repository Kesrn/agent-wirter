...
14. ability_name 必须是具体能力名称，不要带括号注释（如不要写"雷印（雷系初阶技能）"，写"雷印"即可）；不要用泛称（如"冰系初阶技能"应抽为具体技能名）；修炼阶段/掌控进度用 cultivation_level，觉醒系别用 magic_element，不要把不同层级混进同一 ability_name。
